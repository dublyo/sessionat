...
void TabMenuModel::BuildForWebApp(int index) {
  ...
  // Sessionat: "Move to workspace…" submenu, listing every Sessionat workspace
  // plus a "+ New Workspace" option that creates one on the fly.
  if (WorkspaceSubMenuModel::ShouldShowSubmenu(tab_strip_->profile())) {
    sessionat_workspace_submenu_ = std::make_unique<WorkspaceSubMenuModel>(
        delegate(), tab_menu_model_delegate_, tab_strip_, index);
    AddSubMenu(TabStripModel::CommandSessionatMoveToWorkspace,
               u"Move to workspace…", sessionat_workspace_submenu_.get());
  }
