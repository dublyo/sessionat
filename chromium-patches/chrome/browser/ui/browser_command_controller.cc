...
bool BrowserCommandController::ExecuteCommandWithDisposition(
  ...
  switch (id) {
    ...
    case IDC_SELECT_TAB_7: {
      // Sessionat: Cmd+1..8 switches to the Nth WORKSPACE when 2+ workspaces
      // exist; falls back to the upstream "switch to Nth tab" behaviour
      // otherwise (single workspace, classic Chromium UX). Uses the
      // pinned-first ordering so users can rearrange the Cmd+1..9 mapping
      // by pinning their most-used workspaces.
      sessionat::WorkspaceService* ws_service =
          sessionat::WorkspaceServiceFactory::GetForProfile(
              browser_->profile());
      const int index = id - IDC_SELECT_TAB_0;
      if (ws_service) {
        std::vector<sessionat::Workspace> ordered =
            ws_service->GetOrderedWorkspaces();
        if (static_cast<int>(ordered.size()) > index) {
          // Phase 1.4: swap the window's tab set, not just bookmark-open.
          ws_service->SwapToWorkspace(ordered[index].id, browser_);
          base::RecordAction(
              base::UserMetricsAction("Sessionat_SwitchWorkspace"));
          break;
        }
      }
    ...
    case IDC_SELECT_LAST_TAB: {
      // Sessionat: Cmd+9 switches to the LAST workspace (ordered, so the
      // 9th pinned or the last in the storage order) if 2+ exist.
      sessionat::WorkspaceService* ws_service =
          sessionat::WorkspaceServiceFactory::GetForProfile(
              browser_->profile());
      if (ws_service) {
        std::vector<sessionat::Workspace> ordered =
            ws_service->GetOrderedWorkspaces();
        if (ordered.size() >= 2) {
          ws_service->SwapToWorkspace(ordered.back().id, browser_);
          base::RecordAction(
              base::UserMetricsAction("Sessionat_SwitchWorkspaceLast"));
          break;
        }
      }
      base::RecordAction(base::UserMetricsAction("Accel_SelectNumberedTab"));
    ...
    case IDC_SESSIONAT_NEXT_WORKSPACE:
    case IDC_SESSIONAT_PREV_WORKSPACE: {
      // Cycle through workspaces (wraps at both ends). No-op when 0-1 exist.
      sessionat::WorkspaceService* ws_service =
          sessionat::WorkspaceServiceFactory::GetForProfile(
              browser_->profile());
      if (!ws_service) break;
      const auto& all = ws_service->GetAllWorkspaces();
      if (all.size() < 2) break;
      const std::string active = ws_service->GetActiveWorkspaceId();
      int current = -1;
      for (size_t i = 0; i < all.size(); ++i) {
        if (all[i].id == active) { current = static_cast<int>(i); break; }
      }
      const int n = static_cast<int>(all.size());
      const int next = (id == IDC_SESSIONAT_NEXT_WORKSPACE)
                          ? (current + 1 + n) % n
                          : (current - 1 + n) % n;
      // Phase 1.4: swap tabs, not just mark active.
      ws_service->SwapToWorkspace(all[next].id, browser_);
      base::RecordAction(base::UserMetricsAction(
          id == IDC_SESSIONAT_NEXT_WORKSPACE
              ? "Sessionat_NextWorkspace"
              : "Sessionat_PrevWorkspace"));
      break;
    }
    case IDC_SESSIONAT_MOVE_TAB_TO_WORKSPACE: {
      // The spec's "Cmd+Shift+M open picker" — we open the full workspaces
      // manager which lets the user choose a target. True per-tab move-into
      // is a Phase 1.4 follow-up (needs workspace-owned tabs first).
      NavigateParams params(
          browser_, GURL(chrome::kChromeUISessionatWorkspacesURL),
          ui::PAGE_TRANSITION_AUTO_BOOKMARK);
      params.disposition = WindowOpenDisposition::NEW_FOREGROUND_TAB;
      Navigate(&params);
      base::RecordAction(base::UserMetricsAction(
          "Sessionat_OpenWorkspacePicker"));
      break;
    }
    case IDC_SESSIONAT_QUICK_SWITCH: {
      // Cmd+K spotlight. Opens chrome://sessionat-quickswitch/ as a centered
      // popup window. Re-opening when the popup is already up just focuses
      // it (NEW_POPUP for the same host on top of an existing popup will
      // create another; we accept that for v1 and treat it as no-op-ish).
      constexpr int kWidth = 560;
      constexpr int kHeight = 480;
      gfx::Rect anchor = browser_->window()->GetBounds();
      const int x = anchor.x() + (anchor.width() - kWidth) / 2;
      const int y = anchor.y() + std::max(60, anchor.height() / 6);
      NavigateParams params(
          browser_->profile(),
          GURL(chrome::kChromeUISessionatQuickSwitchURL),
          ui::PAGE_TRANSITION_AUTO_TOPLEVEL);
      params.disposition = WindowOpenDisposition::NEW_POPUP;
      params.window_action = NavigateParams::WindowAction::kShowWindow;
      params.window_features.bounds = gfx::Rect(x, y, kWidth, kHeight);
      Navigate(&params);
      base::RecordAction(base::UserMetricsAction("Sessionat_QuickSwitch"));
      break;
    }
...
void BrowserCommandController::InitCommandState() {
  ...
  // Sessionat workspace navigation shortcuts.
  command_updater_.UpdateCommandEnabled(IDC_SESSIONAT_NEXT_WORKSPACE, true);
  command_updater_.UpdateCommandEnabled(IDC_SESSIONAT_PREV_WORKSPACE, true);
  command_updater_.UpdateCommandEnabled(IDC_SESSIONAT_MOVE_TAB_TO_WORKSPACE,
                                        true);
  command_updater_.UpdateCommandEnabled(IDC_SESSIONAT_QUICK_SWITCH, true);
