...
bool IsOriginAllowedServerFallback(const GURL& url) {
  ...
  // Sessionat: the workspace manager + spotlight need real favicons for
  // every saved page, including ones the user has never visited (demo
  // workspace, manually-added URLs). Letting the favicon service hit
  // Google server-side avoids any cross-scheme network fetch from the
  // renderer, which would be killed by the WebUI URL loader factory.
  if (url.GetScheme() == content::kChromeUIScheme) {
    const std::string_view host = url.host();
    if (host == chrome::kChromeUISessionatNtpHost ||
        host == chrome::kChromeUISessionatWorkspacesHost ||
        host == chrome::kChromeUISessionatQuickSwitchHost ||
        host == chrome::kChromeUISessionatAnalyticsHost) {
      return true;
    }
  }
  return false;
