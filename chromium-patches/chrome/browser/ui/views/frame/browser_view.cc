...
bool BrowserView::ShouldDrawTabStrip() const {
  ...
  // Sessionat: when the vertical sidebar is active (default in Sessionat),
  // suppress the horizontal tab strip entirely. The sidebar is the only tab UI.
  if (ShouldDrawVerticalTabStripInternal()) {
    return false;
  }
...
bool BrowserView::ShouldDrawVerticalTabStrip() const {
  // Sessionat: this used to gate on ShouldDrawTabStrip(), but we now want
  // vertical-on => horizontal-off. Express the vertical-strip check directly
  // so the two functions are independent (avoids a cycle).
  return ShouldDrawVerticalTabStripInternal() &&
         horizontal_tab_strip_region_view_ &&
         horizontal_tab_strip_region_view_->tab_strip() != nullptr;
}

bool BrowserView::ShouldDrawVerticalTabStripInternal() const {
  if (!browser_->SupportsWindowFeature(
          Browser::WindowFeature::kFeatureTabStrip)) {
    return false;
  }
