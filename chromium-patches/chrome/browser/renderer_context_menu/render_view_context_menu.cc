...
void RenderViewContextMenu::AppendPageItems() {
  ...
  // Sessionat: "Add page to workspace…" submenu listing every workspace
  // (with item counts) plus a "+ New Workspace" entry. Hidden on chrome://
  // internal pages.
  {
    Profile* profile = Profile::FromBrowserContext(browser_context_);
    GURL page_url = params_.page_url.is_valid() ? params_.page_url
                                                : params_.frame_url;
    if (SessionatPageWorkspaceSubMenu::ShouldShow(profile, page_url)) {
      std::string title;
      if (source_web_contents_) {
        title = base::UTF16ToUTF8(source_web_contents_->GetTitle());
      }
      sessionat_page_workspace_submenu_ =
          std::make_unique<SessionatPageWorkspaceSubMenu>(profile, page_url,
                                                          title);
      menu_model_.AddSubMenu(IDC_SESSIONAT_ADD_PAGE_TO_WORKSPACE,
                             u"Add page to workspace…",
                             sessionat_page_workspace_submenu_.get());
      menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
    }
  }
...
bool RenderViewContextMenu::IsCommandIdEnabled(int id) const {
  ...
  if (id == IDC_SESSIONAT_ADD_PAGE_TO_WORKSPACE) {
    return true;
  }
...
void RenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  ...
  // Sessionat: the IDC_SESSIONAT_ADD_PAGE_TO_WORKSPACE parent now opens a
  // submenu; the individual workspace items handle their own clicks via
  // SessionatPageWorkspaceSubMenu::ExecuteCommand.
  if (id == IDC_SESSIONAT_ADD_PAGE_TO_WORKSPACE) {
    return;
  }
