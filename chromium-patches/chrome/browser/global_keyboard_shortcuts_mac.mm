...
const std::vector<KeyboardShortcutData>& GetShortcutsNotPresentInMainMenu() {
  static const base::NoDestructor<std::vector<KeyboardShortcutData>> keys([]() {
    ...
    // cmd    shift  cntrl  option vkeycode               command
    // ---    -----  -----  ------ --------               -------
      // Sessionat: Cmd+Shift+] / [ cycle WORKSPACES (was: next/prev tab).
      // Tab cycling remains on Ctrl+Tab, Ctrl+PageDown/Up, Cmd+Option+arrows.
      {true,  true,  false, false, kVK_ANSI_RightBracket, IDC_SESSIONAT_NEXT_WORKSPACE},
      {true,  true,  false, false, kVK_ANSI_LeftBracket,  IDC_SESSIONAT_PREV_WORKSPACE},
      ...
      // Sessionat: Cmd+Shift+M opens the workspace picker (was: avatar menu).
      {true,  true,  false, false, kVK_ANSI_M,            IDC_SESSIONAT_MOVE_TAB_TO_WORKSPACE},
      // Sessionat: Cmd+K opens the spotlight-style quick switcher.
      {true,  false, false, false, kVK_ANSI_K,            IDC_SESSIONAT_QUICK_SWITCH},
