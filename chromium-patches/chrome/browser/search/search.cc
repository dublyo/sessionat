...
// Sessionat: only called from the Android-only #else branch of
// NewTabURLDetails::ForProfile; on non-Android the Sessionat NTP URL is
// returned unconditionally before reaching template-URL logic, so this
// helper is unreferenced. is_official_build=true treats unreferenced
// internal-linkage functions as -Werror,-Wunused-function.
#if BUILDFLAG(IS_ANDROID)
...
// Sessionat: see comment on GetDefaultSearchProviderTemplateURL above.
#if BUILDFLAG(IS_ANDROID)
...
#if !BUILDFLAG(IS_ANDROID)
    // Sessionat: route default New Tab Page (Cmd+T, "+" button) to our
    // custom dark NTP with session cards. Returns unconditionally for non-
    // Android; everything below is the Android-only fallback to the original
    // template-URL-driven NTP. Under is_official_build=true the compiler
    // promotes -Wunreachable-code to an error, so the non-Android code
    // below must live inside the #else of this same #if.
    return NewTabURLDetails(GURL(chrome::kChromeUISessionatNtpURL),
                            NEW_TAB_URL_VALID);
#else
