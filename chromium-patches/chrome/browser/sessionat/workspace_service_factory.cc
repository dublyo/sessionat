...
bool WorkspaceServiceFactory::ServiceIsCreatedWithBrowserContext() const {
  // Sessionat: create the WorkspaceService eagerly on profile init so the
  // first-run starter workspaces (Work/Personal/Research) are seeded
  // immediately, without waiting for some UI to call GetForProfile().
  return true;
}
