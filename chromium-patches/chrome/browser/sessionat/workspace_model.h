...
struct WorkspaceItem {
  ...
  static std::optional<WorkspaceItem> FromDict(const base::DictValue& dict);
};
...
struct Workspace {
  ...
  // Whether the user has pinned this workspace to the top. Pinned
  // workspaces are listed first on the NTP, get the leading Cmd+1..9
  // shortcuts, and appear in their own section in the quick switcher.
  ...
  // Convert to/from base::Value for storage.
  base::DictValue ToDict() const;
  static std::optional<Workspace> FromDict(const base::DictValue& dict);
