...
base::Value::Dict WorkspaceItem::ToDict() const {
...
std::optional<WorkspaceItem> WorkspaceItem::FromDict(
  ...
  const base::Value::List* tags_list = dict.FindList(kItemTagsKey);
  if (tags_list) {
...
base::Value::Dict Workspace::ToDict() const {
  base::Value::Dict dict;
...
// static
std::optional<Workspace> Workspace::FromDict(const base::Value::Dict& dict) {
  Workspace workspace;
  ...
  const base::Value::List* tags_list = dict.FindList(kWorkspaceTagsKey);
  if (tags_list) {
    for (const auto& tag_value : *tags_list) {
      if (tag_value.is_string()) {
  ...
  const base::Value::List* items_list = dict.FindList(kWorkspaceItemsKey);
  if (items_list) {
