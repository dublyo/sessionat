...
HorizontalTabStripRegionView::HorizontalTabStripRegionView(
  ...
  // Sessionat: Add workspace button before tab strip.
  // This button allows users to switch between workspaces.
  if (browser &&
      (browser->GetType() == BrowserWindowInterface::Type::TYPE_NORMAL)) {
    auto workspace_button = std::make_unique<WorkspaceTabStripButton>(
        tab_strip_->controller(),
        base::BindRepeating([]() {
          // TODO(sessionat): Show workspace dropdown when clicked.
          // For now, this is a placeholder that will be connected to the
          // extension via chrome.runtime messaging.
        }));
    workspace_button->SetProperty(views::kCrossAxisAlignmentKey,
                                  views::LayoutAlignment::kCenter);
    workspace_button_ = AddChildView(std::move(workspace_button));
  }
...
HorizontalTabStripRegionView::~HorizontalTabStripRegionView() {
  ...
  // Sessionat: Clean up workspace button.
  if (workspace_button_) {
    RemoveChildViewT(std::exchange(workspace_button_, nullptr));
  }
}
...
views::View::Views HorizontalTabStripRegionView::GetChildrenInZOrder() {
  ...
  // Sessionat: Add workspace button first (back of z-order).
  if (workspace_button_) {
    children.emplace_back(workspace_button_.get());
  }
