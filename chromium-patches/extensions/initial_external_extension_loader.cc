...
// Sessionat: Extension ID to always pre-install
...
void InitialExternalExtensionLoader::StartLoading() {
  ...
  // Sessionat: Always include our extension, even if not in preferences
  base::Value::List extended_ids = ids.Clone();
  bool has_sessionat = false;
  for (const auto& id : extended_ids) {
    if (const std::string* str = id.GetIfString()) {
      if (*str == kSessionatExtensionId) {
        has_sessionat = true;
        break;
      }
    }
  ...
  if (!has_sessionat) {
    extended_ids.Append(kSessionatExtensionId);
  }

  LoadFinished(GenerateExtensionPrefs(extended_ids));
}
